...
from src.config import JOB_QUEUE_ARN_GPU, DEFAULT_BUCKET_NAME, JOB_DEFINITION_ARN_URL_DOWNLOAD, USE_AWS
from src.tasks.batch.update import update_job_status
from src.tasks.url_download.tasks import run_url_download_job
...
@router.post("",
    ...
    summary="Submit a URL Download job",
    description="""
Submit a long-running background job for downloading files from URLs.

### Input Parameters (multipart/form-data):
- **url** (`str`, required): URL to download the file from.
...
async def submit_job(
    ...
    try:
        job = JobDocument(type=JobType(name=JobTypeEnum.URL_DOWNLOAD), user_email=current_user.email)
        ...
        if USE_AWS:
            job.outputs = {
                "files": ["url_download/" + job.job_id + "/" + s3.generate_valid_object_name("downloaded_file")]
            }

            response = submit_job(
                JOB_QUEUE_ARN_GPU,
                JOB_DEFINITION_ARN_URL_DOWNLOAD,
                [
                    {"name": "INPUT", "value": url},
            ...
            task = run_url_download_job.delay(job.job_id, url)
        ...
        log.error(f"Error in /url_download: {error_message}")
        raise HTTPException(
            status_code=500,
