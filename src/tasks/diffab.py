...
def run_diffab_job(self, *args):
    ...
    try:
        ...
        job_spec = {
            "image": "loudprepaire/diffab-job:latest",
            "memory": "48g",
            "entrypoint": None,
            "command": [],
            "volumes": {
        ...
        with open(pdb_input_file, "rb") as f:
            content = f.read()

        if not content:
            raise Exception("Generated PDB file is empty.")

        pdb_objectname = "gnina/" + job_id + "/" + s3.generate_valid_object_name("diffab_output.pdb")
        adapter.upload(pdb_output_file, pdb_objectname)

        json_objectname = "gnina/" + job_id + "/" + s3.generate_valid_object_name("diffab_output.json")
