...
def run_musite_job(self, *args):
    ...
    try:
        ...
        job_spec = {
            "image": "loudprepaire/musitdeep-job:latest",
            "memory": "48g",
            "entrypoint": None,
            "command": [],
            "volumes": {
        ...
        json_objectname = "musite/" + job_id + "/" + s3.generate_valid_object_name("musite_output.json")
        adapter.upload(output_file, json_objectname)
        
        job.outputs["files"].append(json_objectname)
        log.info(f"job {job_id} completed, output saved to: {json_objectname}")
        if not self.request.chain:
            job.status = JobStatusEnum.SUCCEEDED.value
        job_repo.update_sync(job)
        self.update_state(state=states.SUCCESS)

        return job_id, json_objectname
